...
struct KdTree
{
	...
	void insertBinaryTree(Node *&node, uint depth, std::vector<float>& point, int id)		// Recursive Function definition		--> Purpose: reassign node in tree, when it is hit & it is NULL			Node *&node is ALREADY DEREFERENCED ---> SHOWS "Test Search" results on terminal		** - double pointer --> memory address pointing at node currently on tree --> dereference it to see what value is
	{
		// Tree is empty
		if (node == nullptr)		// if node is NULL		TERMINATE "insertBinaryTree" function		Better To Use "nullptr" since *&node is technically a pointer
		{
	...
	{
		if (node != nullptr)	// NULL - integer		nullptr - pointer
		{
			...
			{
				float distance = sqrt(pow((target[0] - node->point[0]), 2) + pow((target[1] - node->point[1]), 2));	// find distance between (x, y) NODE & (x, y) TARGET
